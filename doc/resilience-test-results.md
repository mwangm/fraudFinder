...
| T+5m | Identified root cause: stale mutating webhook injecting other init containers |
...
| Root cause | Leftover mutating webhook                                                             |
| Fix applied | Deleted webhook, reduced HPA condition and max replicas to banlance the node vm limit |
